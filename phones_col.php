...
//     $sql = "SELECT * FROM `tb_phones` $limitsql";
    ...
    '.($level==1 ? $level.'<a  href="?view=edit&vals='.$row["id"].'&page=phones" class=" btn btn-warning col-10 m-1 m-sm-0 col-sm-4 ">แก้ไข</a>' : '').'
      '.($level==1 ? $level.'<button name="id" value="'.$row["id"].'"  data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="$(\'#myid\').val(\''.$row["id"].'\'); "  class="btn  btn-danger col-10  m-1 m-sm-0 col-sm-4 ">ลบ</button>' : '').'
 </div>
      </td>
    </tr>'.$tables;
      }
      ...
      require_once("list_phones.html");
        ...
        if ($connectdb->query($sql) === TRUE) {
            ...
            case 'addmsg':
                ...
                $data ="";
                $message=ckstring("message" ,'message ไม่ควรเว้นว่าง' );
                $userid=ckstring("userid" ,'userid ไม่ควรเว้นว่าง' );
             ...
             if(  $message ==false  ){
                ...
                // $birthdate = ckdate($birthdate);
                    $connectdb = connectdb();
                    $sql = "INSERT INTO `tb_message` (`id`, `message` ,`userid` ) VALUES (NULL, '$message' ,$userid);";
