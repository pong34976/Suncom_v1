   ...
   $request["cookie"]["list"] = $_COOKIE;
    ...
    if($_POST["token"]==session_id()){
      if($_POST["name_equip"]==""){
        ...
        $nodata["message"] = "!! ไม่ได้คีย์ name_equip";
        echo json_encode($nodata);
        die();
      }
      add($_POST);
...
function upload($data){
    ...
    if ($_FILES['img_equip']['error'] === UPLOAD_ERR_OK) {
      //uploading successfully done
      } else {
        if($_FILES['img_equip']['error'] != UPLOAD_ERR_NO_FILE){
         $nodata["bg"] = "bg-warning";
  ...
  $row["img_equip"] = 0;
  if(isset($_FILES['img_equip'])){
    $errors= array();
    $file_name = $_FILES['img_equip']['name'];
    $file_size =$_FILES['img_equip']['size'];
    $file_tmp =$_FILES['img_equip']['tmp_name'];
    $file_type=$_FILES['img_equip']['type'];
    $file_ext=strtolower(end(explode('.',$_FILES['img_equip']['name'])));
    ...
    if(in_array($file_ext,$extensions)=== false){
      ...
      $nodata["bg"] = "bg-warning";
      $nodata["message"] = codeToMessage($_FILES['img_equip']['error']);
      echo json_encode($nodata);
 die();
    }
    ...
    $name_equip = trim($data["name_equip"]);
    if(empty($errors)==true){
       move_uploaded_file($file_tmp,"../images/".$name_equip.".jpg");
   
       $row["img_equip"] = "'images/".$name_equip.".jpg'";
    }else{
      $nodata["bg"] = "bg-warning";
      $nodata["message"] = "ไม่สามารถบันทึก ข้อมูลได้ ";
      echo json_encode($nodata);
    }
    return $row["img_equip"];
 }else{
  return 0;
 }
...
function update($data){
 ...
 if($_FILES["img_equip"]["error"] != 4) {
  $row["img_equip"] = upload($data);
  $noteupdate.=",`img_equip` = ".$row["img_equip"]."";
  }else{
  
  }
  $connectdb = connectdb();
  // $sql = "UPDATE `tb_hardware` SET `re_type` = 'ปริ้นเตอร์1', `re_model` = 'HP LaserJet Pro M402-M4031' WHERE `tb_hardware`.`id_hardware` = 1;";
  $sql = "UPDATE `".$table."` SET   $noteupdate WHERE `$table`.`id_equip` = '".$row['id_equip']."' ";
  //  $nodata["bg"] = "bg-warning";
  // $nodata["message"] =   $sql ;
  // echo json_encode($nodata);
  // die();
